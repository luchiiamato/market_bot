...
def calcular_score_inversion(metricas: Dict) -> Tuple[float, List[str]]:
    ...
    pesos = {
        'fuerza_tendencia': 0.20,  # bajé un poco para agregar otros
        'rsi': 0.10,
        'macd': 0.10,
        'bb': 0.10,
        'volumen': 0.10,
        'stochastic': 0.10,
        'momentum': 0.10,
        'soportes_resistencias': 0.05,
        'adx': 0.05,      # nuevo
        'atr': 0.05,      # nuevo
        'cci': 0.05,      # nuevo
    }

    # 1. Fuerza de la Tendencia
    fuerza_tendencia = metricas['fuerza_tendencia']
    score += fuerza_tendencia * 10 * pesos['fuerza_tendencia'] * 4
    razones.append(f"Fuerza de tendencia: {fuerza_tendencia}/3 timeframes alcistas (peso {pesos['fuerza_tendencia']*100:.0f}%)")

    # 2. RSI (igual que antes)
    rsi = metricas['rsi']
    if 30 <= rsi <= 70:
        score += 10 * pesos['rsi']
    ...
    if ratio_volumen > 1.5:
        score += 15 * pesos['volumen']
        razones.append(f"Volumen fuerte ({ratio_volumen:.1f}x)")
    elif ratio_volumen > 1.2:
        score += 10 * pesos['volumen']
        razones.append(f"Volumen moderado ({ratio_volumen:.1f}x)")
    else:
        score += 5 * pesos['volumen']
        razones.append(f"Volumen débil ({ratio_volumen:.1f}x)")

    # 6. Stochastic
    if metricas['stoch_k'] < 20 and metricas['stoch_d'] < 20:
        score += 15 * pesos['stochastic']
        razones.append("Stochastic en sobreventa")
    elif metricas['stoch_k'] > 80 and metricas['stoch_d'] > 80:
        score += 5 * pesos['stochastic']
        razones.append("Stochastic en sobrecompra")
    else:
        score += 10 * pesos['stochastic']
        razones.append("Stochastic neutral")

    # 7. Momentum
    if metricas['momentum'] > 0:
        score += 15 * pesos['momentum']
        razones.append("Momentum positivo")
    else:
        score += 5 * pesos['momentum']
        razones.append("Momentum negativo")

    # 8. Soportes y Resistencias
    if metricas.get('soporte_cercano', False):
        score += 15 * pesos['soportes_resistencias']
        razones.append("Precio cerca de soporte importante")
    elif metricas.get('resistencia_cercana', False):
        score += 5 * pesos['soportes_resistencias']
        razones.append("Precio cerca de resistencia, posible rebote")
    else:
        score += 10 * pesos['soportes_resistencias']
        razones.append("Precio lejos de soportes y resistencias clave")

    # --- Indicadores nuevos ---

    # 9. ADX
    adx = metricas.get('adx', 0)
    if adx >= 25:
        score += 15 * pesos['adx']
        razones.append(f"ADX fuerte ({adx:.1f}), tendencia bien establecida")
    else:
        score += 5 * pesos['adx']
        razones.append(f"ADX débil ({adx:.1f}), tendencia débil o ausente")

    # 10. ATR (volatilidad)
    atr = metricas.get('atr', 0)
    # Suponiendo que un ATR alto indica volatilidad saludable para trading
    if atr > metricas.get('precio', 0) * 0.02:
        score += 15 * pesos['atr']
        razones.append(f"ATR alto ({atr:.2f}), buena volatilidad para movimiento")
    else:
        score += 5 * pesos['atr']
        razones.append(f"ATR bajo ({atr:.2f}), baja volatilidad")

    # 11. CCI
    cci = metricas.get('cci', 0)
    if cci < -100:
        score += 15 * pesos['cci']
        razones.append(f"CCI en sobreventa ({cci:.1f})")
    elif cci > 100:
        score += 5 * pesos['cci']
        razones.append(f"CCI en sobrecompra ({cci:.1f})")
    else:
        score += 10 * pesos['cci']
        razones.append(f"CCI neutral ({cci:.1f})")

    # Ajustes por confirmación de tendencias (igual)
